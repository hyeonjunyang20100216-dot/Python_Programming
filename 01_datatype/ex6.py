...
# a[0] = "J"                           # 문자열 내용 수정이 가능할까? -> TypeError 발생 (불가)
print(type(a[0]))

b = [10, 20, 30]
b[0] = 40                      # 리스트 내용 수정이 가능할까? -> 가능
print(b)
...
print(a)                             # Python
print(b)                             # PYTHON
print(a is b)                        # False (같은 객체인가?)

# 문자열 연결 시 + 연산도 새로운 문자열 객체를 만듦
a = "Hello"
print(id(a))                        # 메모리 주소 출력
a += " Python"
print(id(a))                        # 새로운 메모리 주소 출력 (주소 변경됨)
...
print(next(it))                      # P (인덱스 0을 반환하고, 다음 위치로 이동)
print(next(it))                      # y (인덱스 1을 반환하고, 다음 위치로 이동)
...
print(a[0:6:1])                      # Python 출력 (step이 양수이면 끝까지, 마지막 인덱스 포함)
print(a[::])                         # Python 출력
print(a[::2])                        # Pto 출력 (2칸씩)

print(a[5::-1])                      # nohtyP 출력 (step이 음수이면 처음까지, 0번 인덱스 포함)
print(a[0:5:-1])                     # 출력 안됨 (step이 음수이면 start가 end보다 커야 함)
print(a[5:1:-1])                     # noth 출력

# 인덱싱과 슬라이싱 차이
# print(a[10])                       # IndexError 발생 (범위 벗어남)

print(a[2:100])                      # thon 출력 (슬라이싱은 범위를 벗어나도 에러 미발생)
print(a[10:20])                      # 빈 문자열 "" 출력
...
s = "1301"
print(f"{s[0]}학년 {s[1]}반 {int(s[2]+s[3])}번") 

# 2️⃣ 학번을 슬라이싱만을 이용해서 학년, 반, 번호로 잘라내기
print(f"{s[:1]}학년 {s[1:2]}반 {int(s[2:])}번") 